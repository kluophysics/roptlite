...
//            std::cout << "Delta:" << Delta;//---
            if (rho > 0.75)
            {
                ...
                if (rho < 0.25 && ((ngf1 / (ngf0 + Tolerance) >= Accuracy) || ((tCGstatusSM != TRSM_MIN && tCGstatusSM != TRSM_LCON && tCGstatusSM != TRSM_SCON && tCGstatusSM != TRSM_EXCREGION)) ) ) /*If accurate enough, then always not shrink the region. f2 >= f1 && */
                {
                    Delta *= Shrinked_tau;
...
//            std::cout << ", Delta2:" << Delta << std::endl;//---
            if (rho > Acceptence_Rho ||
                ...
                (ngf1 / (ngf0 + Tolerance) < Accuracy && ((tCGstatusSM == TRSM_MIN || tCGstatusSM == TRSM_LCON || tCGstatusSM == TRSM_SCON || tCGstatusSM == TRSM_EXCREGION ))) /*If accurate enough and the solution is in the trust region, then always accept the new iterate. f2 < f1 || */
                )
            {
...
//            std::cout << "time:" << static_cast<realdp>(getTickCount() - starttime) / CLK_PS << std::endl;//---
            if (Verbose >= ITERRESULT)
            {
                timeSeriesptr[iter] = static_cast<realdp>(getTickCount() - starttime) / CLK_PS;
    ...
    void SolversSMTR::PrintFinalInfo(void)
    {
        ...
        printf("|gf|:%.3e,|gf|/|gf0|:%.3e,time:%.2g,nf:%d,ng:%d,nR:%d,", ngf1, ngf1 / ngf0, ComTime, nf, ng, nR);
        if (nH != 0)
        {
            printf("nH:%d,", nH);
        }
