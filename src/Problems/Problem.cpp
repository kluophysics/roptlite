...
//            std::cout << "fy:" << fy << ":" << fx << std::endl;//---
//            std::cout << fy - fx << ":" << Domain->Metric(x, gfx, xi) << std::endl;//---
//            std::cout << fy - fx - Domain->Metric(x, gfx, xi) << ":" << 0.5 * Domain->Metric(x, xi, Hv) << ":" << Hv.Fnorm() << ":" << xi.Fnorm() << std::endl;//---
    ...
    realdp Problem::g(const Variable &x) const
    {
        return 0;
    };
    ...
    Vector &Problem::StoGrad(const Variable &x, Vector *result, const Vector &batch_index) const
    {
        if (!Domain->GetIsIntrinsic())
            return RieStoGrad(x, result, batch_index);
        
        Vector ExStoGrad(Domain->GetEMPTYEXTR());
        RieStoGrad(x, &ExStoGrad, batch_index);
        Domain->ObtainIntr(x, ExStoGrad, result);
        return *result;
    };

    Vector &Problem::RieStoGrad(const Variable &x, Vector *result, const Vector &batch_index) const
    {
        Vector EStoGrad(*result);
        
        if(NumGradHess)
        {
//            Problem::EucStoGrad(x, &EStoGrad, batch_index).Print("result1:");//---
//            EucStoGrad(x, &EStoGrad, batch_index).Print("result2:");//----
            Domain->EucGradToGrad(x, Problem::EucStoGrad(x, &EStoGrad, batch_index), this, result);
            
            return *result;
        }
        
        Domain->EucGradToGrad(x, EucStoGrad(x, &EStoGrad, batch_index), this, result);
        return *result;
    };

    Vector &Problem::EucStoGrad(const Variable &x, Vector *result, const Vector &batch_index) const
    {
#ifdef SINGLE_PRECISION
        realdp _eps = 1e-4;
#else
        ...
        for (size_t i = 0; i < nn; ++i) {
            x_eps_ptr[i] += _eps;
            x_eps.RemoveAllFromFields();
            double fp = Stof(x_eps, batch_index);
            egf_ptr[i] = (fp - fx) / _eps;
//            std::cout << "i:" << i << ", fp:" << fp << ", fx:" << fx << ", :" << (x_eps - x).Fnorm() << std::endl;//---
            x_eps_ptr[i] = x_ptr[i];
        }
        return *result;
    };
