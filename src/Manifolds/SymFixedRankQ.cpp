...
//        IsVectTranSmooth = true;
    ...
    Vector &SymFixedRankQ::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
        printf("warning:SymFixedRankQ::VecTranDiffRetAdjoint has not been done!\n");
    ...
    {
        if (IsIntrApproach)
        { /* this one is the retraction by projection*/
            Vector exetax(EMPTYEXTR);
            ObtainExtr(x, etax, &exetax);
            
//            return Manifold::Retraction(x, exetax, result);//---
    ...
    Vector &SymFixedRankQ::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
