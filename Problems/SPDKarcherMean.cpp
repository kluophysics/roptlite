...
namespace ROPTLITE{
    ...
    realdp SPDKarcherMean::Stof(const Variable &x, const Vector &batch_index) const
    {
        if (!x.FieldsExist("_L"))
        {
            /* ideally x is a symmetric matrix and this tmp data is not needed.
        ...
        Vector SharedElement(n, n);
        Vector SharedLXL(1, &SharedElement, batch_index.Getlength());
        SharedLXL.NewMemoryOnWrite();
        Vector tmp1(n, n), tmp2(n, n);
        for(integer i = 0; i < batch_index.Getlength(); i++)
        {
            tmp1 = x.Field("_L").TriangleLinSol(Ls.GetElement(batch_index[i]));
            tmp2.AlphaABaddBetaThis(1, tmp1, GLOBAL::N, tmp1, GLOBAL::T, 0);
            SharedLXL.GetElement(i) = tmp2.LogSym();
            SharedLXL.GetElement(i).AddToFields("_EigVec", tmp2.Field("_EigVec"));
            SharedLXL.GetElement(i).AddToFields("_EigVal", tmp2.Field("_EigVal"));
        ...
        realdp result = SharedLXL.DotProduct(SharedLXL) / (static_cast<realdp> (2) * batch_index.Getlength());
        x.AddToFields("logLXL", SharedLXL);
        return result;
    };

    Vector &SPDKarcherMean::EucStoGrad(const Variable &x, Vector *result, const Vector &batch_index) const
    {
        ...
        for(integer i = 0; i < batch_index.Getlength(); i++)
        {
            tmp.AlphaABaddBetaThis(1, SharedLXL.GetElement(i), GLOBAL::N, Ls.GetElement(batch_index[i]), GLOBAL::T, 0);
            result->AlphaXaddThis(1, tmp.TriangleLinSol(Ls.GetElement(batch_index[i]), GLOBAL::T));
        }
        
        *result = result->GetTranspose().TriangleLinSol(x.Field("_L"), GLOBAL::N).TriangleLinSol(x.Field("_L"), GLOBAL::T).GetTranspose();
        result->ScalarTimesThis(1.0 / batch_index.Getlength());
        return *result;
    };
