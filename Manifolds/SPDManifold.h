...
namespace ROPTLITE{
	...
	public:
        ...
        /*Compute the inverse of the exponential mapping: result = R_x^{-1}(y).*/
        virtual Vector &InvExpRetraction(const Variable &x, const Variable &y, Vector *result) const;
        
        /*Compute the inverse of the second-order retraction: result = R_x^{-1}(y).*/
        virtual Vector &InvSecOrdRetraction(const Variable &x, const Variable &y, Vector *result) const;
