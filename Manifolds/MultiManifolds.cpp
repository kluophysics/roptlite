...
namespace ROPTLITE{
	...
	MultiManifolds::MultiManifolds(integer numberofmanifolds, ...)
	{
        ...
        for (integer i = 0; i < numoftypes; i++)
        {
            elements[i] = manifolds[i]->GetEMPTYNORINTR();
        }
        EMPTYNORINTR = Element(numoftypes, elements, powsinterval);
	...
	Vector &MultiManifolds::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
        ...
        for (integer i = 0; i < numoftypes; i++)
        {
            for (integer j = powsinterval[i]; j < powsinterval[i + 1]; j++)
            {
                manifolds[i]->VecTranDiffRetAdjoint(x.GetElement(j), etax.GetElement(j), y.GetElement(j), inxiy.GetElement(j), &(result->GetElement(j)));
            }
        }
        return *result;
	};
	...
	Vector &MultiManifolds::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
        ...
        for (integer i = 0; i < numoftypes; i++)
        {
            for (integer j = powsinterval[i]; j < powsinterval[i + 1]; j++)
            {
                manifolds[i]->VecTranDiffRet(x.GetElement(j), etax.GetElement(j), y.GetElement(j), inxix.GetElement(j), &(result->GetElement(j)), IsEtaXiSameDir);
            }
        }
    ...
    Vector &MultiManifolds::ObtainNorVerIntr(const Variable &x, const Vector &etax, Vector *result) const
    {
        result->NewMemoryOnWrite();
        ...
        for (integer i = 0; i < numoftypes; i++)
        {
            for (integer j = powsinterval[i]; j < powsinterval[i + 1]; j++)
            {
                manifolds[i]->ObtainNorVerIntr(x.GetElement(j), etax.GetElement(j), &(result->GetElement(j)));
            }
        }
        return *result;
    };

    Vector &MultiManifolds::ObtainNorVerExtr(const Variable &x, const Vector &intretax, Vector *result) const
    {
        result->NewMemoryOnWrite();
        ...
        for (integer i = 0; i < numoftypes; i++)
        {
            for (integer j = powsinterval[i]; j < powsinterval[i + 1]; j++)
            {
                manifolds[i]->ObtainNorVerExtr(x.GetElement(j), intretax.GetElement(j), &(result->GetElement(j)));
            }
        }
        return *result;
    };
