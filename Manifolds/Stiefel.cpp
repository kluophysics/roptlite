...
//        IsVectTranSmooth = true;
	...
	Vector &Stiefel::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
		if (retraction == STIE_QF)
		...
		if (retraction == STIE_CAYLEYR)
			return CayleyVecTranDiffRetAdjoint(x, etax, y, xiy, result);
        
		printf("Error: VecTranDiffRetAdjoint has not been done!\n");
        return Projection(x, xiy, result);
	};

	Vector &Stiefel::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
		if (retraction == STIE_QF)
    ...
    Vector &Stiefel::InverseVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result) const
    {
        if(retraction != STIE_POLAR)
        {
            printf("Error: InverseVecTranDiffRetAdjoint has not been done!\n");
            return Manifold::InverseVecTranDiffRetAdjoint(x, etax, y, xix, result);
        }
        
        Vector Vt = y.Field("_Vt"), S = y.Field("_S");
        Vector Vtmp(Vt); S.DiagTimesM(Vtmp); Vtmp = Vt.GetTranspose() * Vtmp;
        Vector YtX = y.GetTranspose() * x;
        Vector Ytxix = y.GetTranspose() * xix;
        Vector B = Ytxix.SYL(YtX, YtX.GetTranspose());
        
        Vector Vtmp2 = (x * (B + B.GetTranspose()) - xix) * Vtmp;
        *result = y * (B * YtX.GetTranspose() * Vtmp + Vtmp * B * YtX.GetTranspose()) - Vtmp2 + y * (y.GetTranspose() * Vtmp2);
        ExtrProjection(y, *result, result);
        return *result;
    };

    Vector &Stiefel::InverseVecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result, bool IsEtaXiSameDir) const
    {
        if(retraction != STIE_POLAR)
        {
            printf("Error: InverseVecTranDiffRet has not been done!\n");
            return Manifold::InverseVecTranDiffRet(x, etax, y, xiy, result, IsEtaXiSameDir);
        }
        
        Vector Vt = y.Field("_Vt"), S = y.Field("_S");
        Vector Vtmp(Vt); S.DiagTimesM(Vtmp); Vtmp = Vt.GetTranspose() * Vtmp;
        Vector Vtmp2 = xiy * Vtmp;
        Vtmp = y.GetTranspose() * Vtmp2;
        Vector P = Vtmp2 - y * Vtmp;
        Vector YtX = y.GetTranspose() * x;
        Vector Vtmp3 = (Vtmp - Vtmp.GetTranspose()) * YtX - x.GetTranspose() * P - P.GetTranspose() * x;
        Vector A = Vtmp3.SYL(YtX.GetTranspose(), YtX);
        *result = y * A + P;
    ...
    Vector &Stiefel::qfVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
    {
    ...
    Vector &Stiefel::InvPolarRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        ...
        if(!IsIntrApproach)
        {
            *result = x; result->AlphaABaddBetaThis(1, y, GLOBAL::N, soln, GLOBAL::N, -1);
            return *result;
        }
        
        Vector extrresult(x);
        extrresult.AlphaABaddBetaThis(1, y, GLOBAL::N, soln, GLOBAL::N, -1); /* y * tIp.SYL(A, A.GetTranspose()) - x; */
        ObtainIntr(x, extrresult, result);
        
        return *result;
    };

	Vector &Stiefel::PolarVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
	...
	Vector &Stiefel::DiffPolarRetraction(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
	{
        ...
        *result = y * Omega + Vtmp2 - y * (y.GetTranspose() * Vtmp2);
	...
	Vector &Stiefel::CayleyVecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
