...
//        IsVectTranSmooth = true;
    ...
    Vector &SPDManifold::InvRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        if (retr == SPDEXP)
            return InvExpRetraction(x, y, result);
        
        if (retr == SPDSECONDORDER)
            return InvSecOrdRetraction(x, y, result);
        
        printf("Warning: the Riemannian retraction is not valid! Use the second order retraction instead!");
        return InvSecOrdRetraction(x, y, result);
    };

    Vector &SPDManifold::VecTranDiffRet(const Variable &x, const Vector &etax, const Variable &y, const Vector &xix, Vector *result, bool IsEtaXiSameDir) const
    {
        if (retr == SPDEXP)
    ...
    Vector &SPDManifold::InvExpRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        if (!x.FieldsExist("_L"))
        {
            x.CholDecom();
        ...
        Vector LiE = y.TriangleLinSol(x.Field("_L"));
        Vector LiELiT = LiE.GetTranspose().TriangleLinSol(x.Field("_L"));
        
        Vector tmp(n, n);
        tmp.AlphaABaddBetaThis(1, x.Field("_L"), GLOBAL::N, LiELiT.LogSym(), GLOBAL::N, 0);
        Vector exresult(EMPTYEXTR);
        exresult.AlphaABaddBetaThis(1, tmp, GLOBAL::N, x.Field("_L"), GLOBAL::T, 0);
        ...
        for(integer i = 0; i < n; i++)
        {
            for(integer j = i + 1; j < n; j++)
            {
                tmpv = (exresultptr[i + j * n] + exresultptr[j + i * n]) / 2;
                exresultptr[i + j * n] = tmpv;
                exresultptr[j + i * n] = tmpv;
            }
        }
        if(!IsIntrApproach)
        {
    ...
    Vector &SPDManifold::InvSecOrdRetraction(const Variable &x, const Variable &y, Vector *result) const
    {
        if (!x.FieldsExist("_L"))
        {
            x.CholDecom();
        ...
        Vector tmp = 2 * y - x;
        Vector LiE = tmp.TriangleLinSol(x.Field("_L"));
        Vector LiELiT = LiE.GetTranspose().TriangleLinSol(x.Field("_L"));
        tmp = LiELiT.SqrtMSym();
        ...
        Vector tmpp(EMPTYEXTR);
        tmpp.AlphaABaddBetaThis(1, x.Field("_L"), GLOBAL::N, tmp, GLOBAL::N, 0);
        Vector exresult(EMPTYEXTR);
        exresult.AlphaABaddBetaThis(1, tmpp, GLOBAL::N, x.Field("_L"), GLOBAL::T, 0);
	...
	Vector &SPDManifold::VecTranDiffRetAdjoint(const Variable &x, const Vector &etax, const Variable &y, const Vector &xiy, Vector *result) const
	{
		printf("SPDManifold::VecTranDiffRetAdjoint has not been done!\n");
